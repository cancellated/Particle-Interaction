...
namespace Mediapipe.Tasks.Vision.GestureRecognizer
{
    ...
    public sealed class GestureRecognizer : Core.BaseVisionTaskApi
    {
        ...
        private const string _TASK_GRAPH_NAME =
...
#pragma warning restore IDE0052
        ...
        private GestureRecognizer(
            ...
            Tasks.Core.TaskRunner.PacketsCallback packetCallback
        )
            : base(graphConfig, runningMode, gpuResources, packetCallback)
        {
        ...
        /// </returns>
        public static GestureRecognizer CreateFromModelPath(
            string modelPath,
        ...
        )
        {
            ...
            var options = new GestureRecognizerOptions(
                baseOptions,
                runningMode: Core.RunningMode.IMAGE
            );
        ...
        public GestureRecognizerResult Recognize(
            Image image,
            Core.ImageProcessingOptions? imageProcessingOptions = null
        )
        {
        ...
        public bool TryRecognize(
            Image image,
            Core.ImageProcessingOptions? imageProcessingOptions,
        ...
        private PacketMap RecognizeInternal(
            Image image,
            Core.ImageProcessingOptions? imageProcessingOptions
        )
        {
            ConfigureNormalizedRect(
                _normalizedRect,
        ...
        {
            using var outputPackets = RecognizeForVideoInternal(
                image,
                timestampMillisec,
                imageProcessingOptions
            );
        ...
        ///   <see langword="true"/> if some gestures are detected, <see langword="false"/> otherwise.
        /// </returns>
        public bool TryRecognizeForVideo(
            Image image,
            long timestampMillisec,
            Core.ImageProcessingOptions? imageProcessingOptions,
            ref GestureRecognizerResult result
        ...
        private PacketMap RecognizeForVideoInternal(
            Image image,
            long timestampMillisec,
            Core.ImageProcessingOptions? imageProcessingOptions = null
        )
        {
            ...
            packetMap.Emplace(
                ...
                Packet.CreateImageAt(image, timestampMicrosec)
            );
            packetMap.Emplace(
                _NORM_RECT_STREAM_NAME,
                Packet.CreateProtoAt(_normalizedRect, timestampMicrosec)
            );
        ...
        private static Tasks.Core.TaskRunner.PacketsCallback BuildPacketsCallback(
            GestureRecognizerOptions options
        )
        {
            ...
            return (PacketMap outputPackets) =>
            {
                ...
                var timestamp =
                    outImagePacket.TimestampMicroseconds() / _MICRO_SECONDS_PER_MILLISECOND;
        ...
        private static bool TryBuildGestureRecognizerResult(
            PacketMap outputPackets,
            ref GestureRecognizerResult result
        )
        {
            using var gesturesPacket = outputPackets.At<List<Classifications>>(
                _HAND_GESTURE_STREAM_NAME
            );
            ...
            using var handednessPacket = outputPackets.At<List<Classifications>>(
                _HANDEDNESS_STREAM_NAME
            );
            var handedness = result.handedness ?? new List<Classifications>();
            handednessPacket.Get(handedness);

            using var handLandmarksPacket = outputPackets.At<List<NormalizedLandmarks>>(
                _HAND_LANDMARKS_STREAM_NAME
            );
            var handLandmarks = result.handLandmarks ?? new List<NormalizedLandmarks>();
            handLandmarksPacket.Get(handLandmarks);

            using var handWorldLandmarksPacket = outputPackets.At<List<Landmarks>>(
            ...
            result = new GestureRecognizerResult(
                gestures,
                handedness,
                handLandmarks,
                handWorldLandmarks
